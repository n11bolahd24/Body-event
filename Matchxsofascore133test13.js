document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderMatch(matchId, matchKey) {
        ...
        if (!container) {
            console.error("Container matchesContainer tidak ditemukan!");
            return;
        }
        ...
        // Delay sedikit agar DOM siap sebelum load
        setTimeout(() => loadSofaScore(matchId, matchKey), 50);
    }

    // --- Fungsi Utama Load Sofascore + Countdown ---
    async function loadSofaScore(matchId, boxId) {
        const matchStatusEl = document.getElementById("matchStatus" + boxId);
        matchStatusEl.innerHTML = "Loading...";
        try {
            const res = await fetch(`https://api.sofascore.com/api/v1/event/${matchId}`);
            const data = await res.json();
            const event = data.event;
            if (!event) {
                matchStatusEl.innerHTML = "Match data not available";
                return;
            }
            ...
            console.error("Gagal load matchId", matchId, err);
            matchStatusEl.innerHTML = "Error loading match";
        }
    }
    ...
    function monitorMatchStatusRealtime(matchId, boxId, kickoffTime) {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                console.error("Error update match", matchId, err);
            }
        }, 2000);
    }
    ...
    // --- Contoh pemanggilan match ---
    // Staggered agar semua match tampil aman
    const matches = [
        {id: 12345, key: 1},
        {id: 12346, key: 2},
        {id: 12347, key: 3},
        // Tambahkan match sesuai kebutuhan
    ];
    matches.forEach((match, index) => {
        setTimeout(() => renderMatch(match.id, match.key), index
