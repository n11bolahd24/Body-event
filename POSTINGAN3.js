...
function loadSofaScore(matchId, boxId) {
    ...
    fetch(eventUrl)
        ...
        .catch(err => {
            console.warn("⚠️ Sofascore gagal → pakai fallback", err);

            // ===== FALLBACK LEAGUE + KICKOFF + COUNTDOWN =====
            const matchBox = document.getElementById("match" + boxId);
            if (!matchBox) return;

            // fallback league
            const leagueEl = document.getElementById("league" + boxId);
            if (leagueEl && matchBox.dataset.league) {
                leagueEl.innerHTML = `<span>${matchBox.dataset.league}</span>`;
            }

            // fallback kickoff + countdown
            if (matchBox.dataset.kickoff) {
                const fallbackDate = new Date(
                    matchBox.dataset.kickoff.replace(" ", "T")
                );

                const tgl = fallbackDate.toLocaleDateString(undefined, {
                    day: '2-digit',
                    month: 'long',
                    year: 'numeric'
                });

                const jm = fallbackDate.toLocaleTimeString(undefined, {
                    hour: '2-digit',
                    minute: '2-digit',
                    hour12: false
                });
                ...
                startCountdown(fallbackDate.getTime(), boxId);
            }
        });
}
                ...
                if (event.status.type === "penalties") {
                    ...
                    switch (event.status.description) {
                        case "1st half":
                            ...
                            break;
                        case "2nd half":
                            statusText = (45 + elapsed) >= 90 ? "90+'" : `${45 + elapsed}'`;
                            break;
                        case "1st extra":
                            statusText = (90 + elapsed) >= 105 ? "105+'" : `${90 + elapsed}'`;
                            break;
                        case "2nd extra":
                            statusText = (105 + elapsed) >= 120 ? "120+'" : `${105 + elapsed}'`;
                            break;
                        default:
