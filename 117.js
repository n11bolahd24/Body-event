...
function renderMatch(matchId, matchKey, serverFuncs, boxClass = "kotak") {
  ...
  <div class="${boxClass}" id="match${matchKey}" class="kotak matchbox">
    ...
    <!-- kickoff countdown sofascore (TETAP ADA) -->
    ...
    <div class="club">
      <center>
        <span id="league${matchKey}" style="position:relative; top:5px; left:-11px; font-weight:bold; font-size:12px; color:white;">
          NAMA LIGA
        </span>
        ...
        <font id="teams${matchKey}" style="font-size:15px; font-weight:bold">
          NAMA CLUB VS NAMA CLUB
        </font><br>
        <div id="kickoff${matchKey}" 
             style="font-size:12px; color:white; text-align:center; margin:1px 0; font-style:italic;"></div>
      </center>
    </div>
    ...
    <!-- countdown TV server baru -->
    <center>
      <div id="countdownTV${matchKey}" 
           style="font-size:13px; color:yellow; font-weight:bold; margin-top:8px;">
      </div>
...
// --- fungsi untuk countdown TV server (TERPISAH dari SofaScore) ---
function activateTVServerAt(matchKey, targetTimeString) {
  ...
  function updateCountdown() {
    ...
    if (diff <= 0) {
      countdownEl.innerHTML = "🎬 TV Server Aktif Sekarang!";
  ...
  // disable tombol selama countdown
