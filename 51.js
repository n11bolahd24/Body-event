...
function monitorMatchStatus(matchId, boxId) {
  ...
  const interval = setInterval(async () => {
    ...
    if (event.status.type === "inprogress") {
      // Saat pertandingan berlangsung → LIVE NOW
      ...
      // Pertandingan selesai → MATCH ENDED
      clearInterval(interval);
      countdownEl.innerHTML = "";
      liveContainer.classList.remove('blink');    // hapus blink
      liveContainer.style.animation = "none";     // hentikan animasi
      liveContainer.classList.remove('hidden');   // pastikan tetap tampil
      liveContainer.innerHTML = "<strong style='color:white;-webkit-text-stroke: 0.2px black;'>⛔ MATCH ENDED ⛔</strong>";

      if (finishedContainer) finishedContainer.appendChild(matchBox);
    }
  }, 1000);
}
