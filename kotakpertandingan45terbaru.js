...
// --- fungsi tambahan untuk render pertandingan dengan countdown TV server ---
function renderMatch(matchId, matchKey, serverFuncs, boxClass = "kotak", startTime = null) {
  const html = `
  <div class="${boxClass}" id="match${matchKey}" class="kotak matchbox">
    <div class="countdown" id="countdown${matchKey}" style="font-size:16px; color:yellow; text-align:center;"></div>
    ...
    <div class="club">
      <center>
        ...
        <div id="liveScore${matchKey}" style="font-size:20px; font-family:'Arial'; font-weight:bold; color:orange;"></div>  
        <div id="matchStatus${matchKey}" style="font-family:'Courier New'; font-size:10px; font-weight:bold; color:orange;"></div>   
    ...
    <center>
      <span id="tvContainer${matchKey}" style="font-size: large; display:none;">
        ${serverFuncs.map((fn, i) => `
          <a class="tv" href="javascript:${fn}();" style="margin:0 3px;"><b><span>SERVER ${i + 1}</span></b></a>
        `).join(" ")}
      </span>
    </center>
    <br>
  ...
  // kalau ada waktu mulai, jalankan countdown
  if (startTime) {
    startCountdown(matchKey, startTime);
  } else {
    document.getElementById(`tvContainer${matchKey}`).style.display = "inline-block";
  }
}
