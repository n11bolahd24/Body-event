...
function loadSofaScore(matchId, boxId, region = "Asia/Jakarta") {
  ...
  fetch(eventUrl)
    ...
    .then(data => {
      ...
      // Pindahkan ke Match Ended setelah 2 jam
      moveToMatchEnded(kickoffDate.getTime(), boxId);
    });
...
// --- Fungsi Pindahkan ke Match Ended ---
function moveToMatchEnded(kickoffTime, boxId) {
  var matchBox = document.getElementById("matchBox" + boxId);
  var finishedContainer = document.getElementById("finishedMatches");

  if (!matchBox || !finishedContainer) return;

  var matchEndTime = kickoffTime + (2 * 60 * 60 * 1000); // +2 jam setelah kickoff
  ...
  var interval = setInterval(function () {
    ...
    if (now >= matchEndTime) {
      ...
      if (matchBox.parentNode.id !== "finishedMatches") {
        ...
        var statusEl = document.getElementById("status" + boxId);
        var countdownEl = document.getElementById("countdown" + boxId);
        if (statusEl) statusEl.innerText = "Match Ended";
        if (countdownEl) countdownEl.innerText = "";
        var liveEl = document.getElementById("liveContainer" + boxId);
        if (liveEl) liveEl.classList.add("hidden");
        if (countdownEl) countdownEl.classList.remove("blink");
      }
    }
  }, 1000);
}
