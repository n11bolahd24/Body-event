...
// (versi dengan statistik Ball Possession di kiri atas)

function loadSofaScore(matchId, matchKey) {
  console.log("Load SofaScore untuk matchId=" + matchId + " key=" + matchKey);

  // Ambil data statistik dari Sofascore API
  fetch(`https://api.sofascore.com/api/v1/event/${matchId}/statistics`)
    ...
    .then(data => {
      ...
      if (home && away) {
        possessionEl.innerHTML = `⚽ ${home}% - ${away}%`;
      } else {
        possessionEl.innerHTML = `⏳ waiting data`;
      }
    })
    .catch(err => {
      console.error(err);
      const possessionEl = document.getElementById(`possession${matchKey}`);
      if (possessionEl) possessionEl.innerHTML = "⚠️ error";
    });
}
...
function renderMatch(matchId, matchKey, serverFuncs, boxClass = "kotak", tvServerTime = null) {
  ...
  <!-- 🔹 Ball Possession (kiri atas) -->
  <div id="possession${matchKey}" 
       style="position:absolute; top:6px; left:8px; font-size:11px; font-weight:bold; color:cyan;">
       ⏳
  </div>
